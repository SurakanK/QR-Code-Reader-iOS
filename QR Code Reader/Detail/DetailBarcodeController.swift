...
class DetailBarcodeController: UIViewController {
    ...
    var imageBarcode = #imageLiteral(resourceName: "Barcode")
    var imageicon = #imageLiteral(resourceName: "Barcode")
    ...
    let text_Barcode: UILabel = {
        let label = UILabel()
        label.text = "Barcode : 501545478752233"
    ...
    let image_Barcode: UIImageView = {
        let image = UIImageView()
        image.image = #imageLiteral(resourceName: "Barcode")
    ...
    let btn_SearchonWeb: UIButton = {
        let button = UIButton(type: .system)
        button.backgroundColor = UIColor.AccentColor
        button.layer.cornerRadius = 10
        button.setTitle("Search on Web", for: .normal)
        button.setTitleColor(UIColor.black, for: .normal)
        button.titleLabel?.font = UIFont.NunitoSansExtraBold(size: 17)
        ...
        button.layer.shadowRadius = 2
        button.layer.shadowOpacity = 6
        button.addTarget(self, action: #selector(SearchonWebButtonClicker), for: .touchUpInside)
        return button
    }()
    ...
    func SettingNavigetion(){
        ...
        navigationItem.rightBarButtonItems = [rightBarButtonDelete]
    }
    ...
    func SettingElement(){
        ...
        text_Barcode.anchor(text_Subject.bottomAnchor, left: view_Discription.leftAnchor, bottom: nil, right: view_Discription.rightAnchor, topConstant: 15, leftConstant: 15, bottomConstant: 0, rightConstant: 15, widthConstant: 0, heightConstant: 0)
        text_Date.anchor(text_Barcode.bottomAnchor, left: view_Discription.leftAnchor, bottom: view_Discription.bottomAnchor, right: view_Discription.rightAnchor, topConstant: 5, leftConstant: 15, bottomConstant: 15, rightConstant: 15, widthConstant: 0, heightConstant: 0)
        
        view.addSubview(image_Barcode)
        image_Barcode.anchor(view_Discription.bottomAnchor, left: nil, bottom: view_QRCode.bottomAnchor, right: nil, topConstant: 20, leftConstant: 0, bottomConstant: 20, rightConstant: 0, widthConstant: 0, heightConstant: 0)
        image_Barcode.anchorCenter(view_QRCode.centerXAnchor, AxisY: nil, ConstantAxisX: 0, ConstantAxisY: 0, widthConstant: 150, heightConstant: 150)
        
        view.addSubview(btn_SearchonWeb)
        btn_SearchonWeb.anchor(view_QRCode.bottomAnchor, left: view.leftAnchor, bottom: nil, right: view.rightAnchor, topConstant: 15, leftConstant: 20, bottomConstant: 0, rightConstant: 20, widthConstant: 0, heightConstant: 50)
    }
    ...
    func SettingGoogleAdsBanner(){
        ...
        Google_Ads_banner.anchor(btn_SearchonWeb.bottomAnchor, left: view.leftAnchor, bottom: view.safeAreaLayoutGuide.bottomAnchor, right: view.rightAnchor, topConstant: 30, leftConstant: 20, bottomConstant: 10, rightConstant: 20, widthConstant: 0, heightConstant: 0)
        
    }
    
    func Process(){
        
        if StateView == "Scane" && navigationItem.rightBarButtonItems!.count > 0{
            ...
            text_Barcode.text = "Barcode : " + TextBarcode
            image_Barcode.image = #imageLiteral(resourceName: "Barcode")
            ...
            databaseHelper.insertData(subjectVal: "",
                                      ...
                                      imageQrVal: (imageBarcode.pngData())!,
                                      dateVal: Date,
                                      IDlistVal: String(ID),
                                      imageiconVal: imageicon.pngData()!,
                                      TextScaneVal: TextBarcode,
                                      isCreate: false)
            ...
            image_Barcode.image = imageBarcode
            text_Barcode.text = TextBarcode
    ...
    @objc func SearchonWebButtonClicker(){
        
        let TextUrl = "http://www.google.com/search?q=\(TextBarcode)"
        
        if URL(string: TextUrl) != nil{
...
extension DetailBarcodeController : GADBannerViewDelegate{
